...
class SJF(Escalation):
    def pick(self):
        if self.readyLine == []:
            return
        chosen = self.readyLine[0]
        for process in self.readyLine:
            if process.getRemainingTime() < chosen.getRemainingTime():
                chosen = process
    ...
    def start(self):
        ...
        while self.processes != [] or self.execLine != [] or self.readyLine != [] or self.waitingLine != []:
            ...
            if self.execLine == []:
                ...
                #Executando a preempcao:
                for process in self.readyLine:
                    #Se existe um processo na fila de prontos com menor tempo de Turnaround
                    if process.getRemainingTime() < self.execLine[0].getRemainingTime():
                        #Entao move o processo em execucao pra fila de prontos
                        self.readyLine.append(self.execLine[0])
                        #Escalona o novo processo
                        self.execLine[0] = process
                        #e o remove da fila de prontos
                if self.execLine[0] in self.readyLine:
                    self.readyLine.remove(self.execLine[0])
